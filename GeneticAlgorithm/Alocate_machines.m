...
while Berth_qt_aux
	...
	% actual ships in the position of interest
	ships = aloc_N(Position);
    
    % first optimize to find first reference values
    [~,T,t,~,~] = ConstroiModelo(Berth_qt_aux, Machine_qt, max_M, min_M, ...
        Ship_cargo(ships), rate, M, Arrival_time(ships), T_berth_max, [], []);
	...
	% subset of concorrent ships
	ships_Set = ships(Set);
	
    %second optimization, only with concorrent ships
    [m,T,t,f,~] = ConstroiModelo(Ship_qt_aux, Machine_qt, max_M, min_M, ...
        Ship_cargo(ships_Set), rate, M, Arrival_time(ships_Set), T_berth_max, [], []);
    
	% Ordenates the berth time of ships of interest
    [~,I_ord] = sort(T_berth(Ship_berth(ships_Set)));
	T_berth_ordered = T_berth(Ship_berth(ships_Set(I_ord)));
    
    T_berth_aux(1:Ship_qt_aux) = T_berth_ordered(Ship_qt_aux);
    %find the best time
    A_add = []; b_add = [];
    for i = Ship_qt_aux:-1:1
        ...
        for j = 0:Machine_qt-1
            %if(sum(M_berth(Ship_berth(aloc_N(Position(Set(I_ord(1:i)))))))>0)
                ...
                b_aux(j+1) = sum(M_berth(j+1,...
                    find(T_berth < max(T_berth_ordered(i)), Arrival_time(ships_Set(I_ord(i))));
            %end
        end
		...
		for j = 1:i
            T_berth_aux1(j) = T_berth_ordered(i);
        end
		
        [m_aux,T_aux,t_aux,f_aux,exitflag] = ConstroiModelo(Ship_qt_aux, Machine_qt, max_M, min_M, ...
            Ship_cargo(ships_Set), rate, M, Arrival_time(ships_Set), T_berth_aux1, [A_add; a], [b_add; b_aux]);
        ...
        if(f_aux < f)
            ...
            for j = 1:i
                T_berth_aux(j) = T_berth_ordered(i);
            end
        end
    end
    ...
    t_Ships(ships_Set) = t;
    T_Ships(ships_Set) = T;
    T_berth(ships_Set) = T_Ships(ships_Set)+t_Ships(ships_Set);
 
    M_berth(:,Ship_berth(ships_Set)) = m_Ships(:,ships_Set);
