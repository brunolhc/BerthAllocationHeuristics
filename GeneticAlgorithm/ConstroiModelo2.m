function[m,T,t,f,exitflag] = ConstroiModelo2(Ship_qt, Machine_qt, max_M, min_M, ...
    Ship_cargo, tax, M, Arrival_time, Berth_time, A_add, b_add)

%x = solution vector (m[i,alpha], T[i], t[i], z[i])

%addpath('C:\Program Files\IBM\ILOG\CPLEX_Studio126\cplex\matlab\x64_win64');
max_machine = max(max_M);

A = zeros(Machine_qt+Machine_qt*Ship_qt*max_machine+Ship_qt, Ship_qt*(Machine_qt+3));
b = zeros(Machine_qt+Machine_qt*Ship_qt*max_machine+Ship_qt, 1);
c = zeros(1,Ship_qt*(Machine_qt+3));
ub = 1000*ones(Ship_qt*(Machine_qt+3),1);
lb = zeros(Ship_qt*(Machine_qt+3),1);
ctype = char(Ship_qt*(Machine_qt+3),1);
...
for i = 1:Ship_qt
    A(lin,i+Ship_qt*(Machine_qt+2)) = 1;
    for j = 1:Machine_qt
        for k = 2:sum(M)
            A(lin,i+(j-1)*Ship_qt) = -Ship_cargo(i)/(tax(j)*(k-1)) + Ship_cargo(i)/(tax(j)*k);
            b(lin) = -k*(Ship_cargo(i)/(tax(j)*(k-1)) - Ship_cargo(i)/(tax(j)*k)) - Ship_cargo(i)/(tax(j)*k);
